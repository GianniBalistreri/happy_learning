...
from easyexplore.unsupervised_machine_learning import UnsupervisedML
from easyexplore.utils import EasyExploreUtils, Log, SPECIAL_CHARACTERS, SPECIAL_SEPARATORS
...
class TextMiner:
    ...
    def __init__(self,
        ...
        if kwargs.get('dask_client') is None:
            self.dask_client = HappyLearningUtils().dask_setup(client_name='text_miner',
        ...
        if isinstance(df, pd.DataFrame):
            self.df: dd.core.DataFrame = dd.from_pandas(data=df, npartitions=self.partitions)
    ...
    def _apply_clustering(self, features: List[str], k: int = 3):
        ...
        for feature in features:
            ...
            _clean_text_data: pd.DataFrame = dd.from_array(x=np.array([self.clean_text(phrase=str(phrase)) for phrase in self.df[feature].values.compute()]))
    ...
    def _apply_similarity(self, features: List[str]):
        ...
        for feature in features:
            if feature in self.df.columns:
                ...
                for x, case in enumerate(self.df[feature].values.compute()):
                    ...
                    if isinstance(case, str):
                        ...
                        for other_case in self.df[feature].values.compute():
                            if case != other_case:
                ...
                if _sep is None:
                    ...
                    if _split_features.shape[1] <= 1:
                        ...
                        for sf in _split_features.keys():
                            ...
                            self.df[sf] = dd.from_array(x=_split_features[sf].values)
    ...
    def tfidf(self, features: List[str] = None):
        ...
        for feature in _features:
            if feature in self.df.columns:
                ...
                _clean_text_data: dd.DataFrame = dd.from_array(x=np.array([self.clean_text(phrase=str(phrase)) for phrase in self.df[feature].values.compute()]))
                self.generated_features[feature]['tfidf'].append('{}_tfidf'.format(feature))
                self.df[self.generated_features[feature]['tfidf'][-1]] = dd.from_array(x=_tfidf_vectorizer.fit_transform(_clean_text_data.values.compute()).data[0:len(self.df)])
